...
<div class="space-y-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="relative rounded-[2rem] overflow-hidden bg-slate-900 border border-slate-800 shadow-xl shadow-slate-900/20">
        ...
        <div class="relative z-10 px-8 py-10 md:px-12 md:py-14 flex items-center gap-5">
            <div class="w-16 h-16 rounded-2xl bg-slate-800 border border-slate-700 flex items-center justify-center shadow-lg shrink-0">
                <svg class="w-8 h-8 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
            </div>
            <div>
                <h1 class="text-white text-2xl md:text-4xl font-black mb-1 drop-shadow-sm">Agenda Kegiatan</h1>
                <p class="text-slate-400 text-sm md:text-base font-medium">Jadwal kegiatan dan acara kebersamaan warga RT 08 RW 02</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-[2rem] border border-slate-200 shadow-sm p-12 text-center max-w-2xl mx-auto mt-8">
        <div class="w-24 h-24 bg-blue-50 border border-blue-100 rounded-full flex items-center justify-center mx-auto mb-6">
            <svg class="w-12 h-12 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pt-4">
        ...
        @php
            ...
            $dateBg = $isPast ? 'bg-slate-50' : 'bg-blue-50';
            $dateMonth = $isPast ? 'text-slate-400' : 'text-blue-600';
            $dateDay = $isPast ? 'text-slate-500' : 'text-blue-700';
        @endphp
        
        <a href="{{ route('warga.kegiatan.show', $item) }}" class="bento-card-hover {{ $cardBg }} rounded-[1.5rem] border {{ $isToday ? 'border-blue-300 shadow-md shadow-blue-500/10 ring-1 ring-blue-50' : 'border-slate-200 shadow-sm' }} flex flex-col group overflow-hidden relative">
            
            @if($isToday)
            <div class="absolute -right-10 -top-10 w-32 h-32 bg-blue-100/50 rounded-full blur-2xl z-0 pointer-events-none"></div>
            @endif
            
            <div class="absolute top-0 left-0 w-1 h-full {{ $isToday ? 'bg-blue-600' : 'bg-slate-200' }} group-hover:bg-blue-500 transition-colors duration-300 z-10 pointer-events-none"></div>

            @if($item->foto)
            <div class="h-48 w-full overflow-hidden relative shrink-0 z-0 bg-slate-100 border-b border-slate-100">
                <img src="{{ Storage::url($item->foto) }}" alt="{{ $item->nama_kegiatan }}" class="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-700">
            </div>
            @endif
            ...
            <div class="p-5 flex-1 flex flex-col relative z-10 pl-6">
                <div class="flex items-start gap-4 mb-5">
                    {{-- Calendar Badge --}}
                    <div class="w-14 h-14 rounded-xl {{ $dateBg }} border border-slate-100 flex flex-col items-center justify-center shrink-0 transition-transform group-hover:scale-105">
                    ...
                    <div class="flex-1 min-w-0 pt-1">
                        <div class="flex items-center gap-2 flex-wrap mb-1.5">
                            @if($isToday)
                            <span class="text-[10px] font-black bg-blue-100 text-blue-700 px-2.5 py-1 rounded-md animate-pulse tracking-widest uppercase">Hari Ini</span>
                            @elseif($isPast)
                            ...
                            @else
                            <span class="text-[10px] font-black bg-blue-50 text-blue-600 px-2.5 py-1 rounded-md uppercase tracking-wider border border-blue-100">Mendatang</span>
                            @endif
                        </div>
                        <h3 class="text-lg font-black {{ $textColor }} leading-snug transition-colors line-clamp-2 group-hover:text-blue-600">{{ $item->nama_kegiatan }}</h3>
                    </div>
                </div>
                
                <div class="space-y-3 mb-6 flex-1 mt-2">
                    <div class="flex items-start gap-3">
                        <div class="w-6 h-6 rounded-md bg-slate-50 border border-slate-100 flex items-center justify-center shrink-0 mt-0.5">
