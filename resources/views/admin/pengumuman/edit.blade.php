...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
            ...
            <a href="{{ route('admin.pengumuman.show', $pengumuman) }}" class="text-sm text-blue-600 hover:underline">← Detail</a>
        </div>
        ...
        <form method="POST" action="{{ route('admin.pengumuman.update', $pengumuman) }}" class="px-6 py-6 space-y-5">
            ...
            <div>
                ...
                <input type="text" id="judul" name="judul" value="{{ old('judul', $pengumuman->judul) }}"
                       class="w-full px-4 py-2.5 text-sm rounded-xl border @error('judul') border-red-400 bg-red-50 @else border-slate-200 @enderror focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div>
                ...
                <input type="date" id="tanggal" name="tanggal" value="{{ old('tanggal', $pengumuman->tanggal) }}"
                       class="w-full sm:w-48 px-4 py-2.5 text-sm rounded-xl border @error('tanggal') border-red-400 bg-red-50 @else border-slate-200 @enderror focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div>
                ...
                <textarea id="isi" name="isi" rows="8"
                          class="w-full px-4 py-3 text-sm rounded-xl border @error('isi') border-red-400 bg-red-50 @else border-slate-200 @enderror focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none leading-relaxed">{{ old('isi', $pengumuman->isi) }}</textarea>
