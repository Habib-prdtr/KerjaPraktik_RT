        ...
        <form method="POST" action="{{ route('admin.kartu-keluarga.store') }}" class="px-6 py-6 space-y-5">
            ...
            <div>
                ...
                <textarea id="alamat" name="alamat" rows="3" placeholder="Alamat lengkap"
                          class="w-full px-4 py-2.5 text-sm rounded-xl border @error('alamat') border-red-400 bg-red-50 @else border-slate-200 @enderror focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none">{{ old('alamat') }}</textarea>
                @error('alamat')
                <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label for="rt" class="block text-sm font-medium text-slate-700 mb-1.5">
                        RT <span class="text-red-500">*</span>
                    </label>
                    <input type="text" id="rt" name="rt" value="{{ old('rt', '08') }}"
                ...
                <div>
                    <label for="rw" class="block text-sm font-medium text-slate-700 mb-1.5">
                        RW <span class="text-red-500">*</span>
                    </label>
                    <input type="text" id="rw" name="rw" value="{{ old('rw', '02') }}"
