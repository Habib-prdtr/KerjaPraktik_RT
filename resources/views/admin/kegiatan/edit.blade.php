...
@section('title', 'Edit Kegiatan — Admin RT 08')
@section('page-title', 'Edit Kegiatan RT')
@section('page-subtitle', 'Perbarui data kegiatan')
        ...
        <form method="POST" action="{{ route('admin.kegiatan.update', $kegiatan) }}" enctype="multipart/form-data" class="px-6 py-6 space-y-5">
            ...
            <div>
                <label for="foto" class="block text-sm font-medium text-slate-700 mb-1.5">
                    Ganti Foto Dokumentasi <span class="text-slate-400 text-xs">(opsional)</span>
                </label>
                @if($kegiatan->foto)
                    <div class="mb-3">
                        <p class="text-xs text-slate-500 mb-1">Foto saat ini:</p>
                        <img src="{{ Storage::url($kegiatan->foto) }}" alt="Foto Kegiatan" class="w-32 h-24 object-cover rounded-xl border border-slate-200">
                    </div>
                @endif
                ...
                <div id="image-preview-container" class="mt-3 hidden">
                    <p class="text-xs text-emerald-600 mb-1 font-semibold">Preview foto baru:</p>
                    <img id="image-preview" src="" alt="Preview Foto Baru" class="max-w-xs rounded-xl border border-emerald-200 object-cover shadow-sm">
                </div>
