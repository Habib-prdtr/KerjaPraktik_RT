...
<body class="bg-slate-50 text-slate-800 antialiased overflow-x-hidden selection:bg-emerald-500 selection:text-white">
    ...
    <nav class="fixed w-full z-50 transition-all duration-300 backdrop-blur-md bg-white/70 border-b border-slate-200/50" id="navbar">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-20">
                ...
                <div class="hidden md:flex items-center space-x-6">
                    ...
                    <a href="#agenda" class="text-sm font-bold text-slate-600 hover:text-emerald-600 transition-colors">Agenda Kegiatan</a>
                    @auth
                        <a href="{{ route('warga.surat.index') }}" class="text-sm font-bold text-slate-600 hover:text-emerald-600 transition-colors">Surat Menyurat</a>
                        <a href="{{ route('warga.pengaduan.index') }}" class="text-sm font-bold text-slate-600 hover:text-emerald-600 transition-colors">Pengaduan</a>
                    @else
                        <button onclick="showAuthModal()" class="text-sm font-bold text-slate-600 hover:text-emerald-600 transition-colors focus:outline-none">Surat Menyurat</button>
                        <button onclick="showAuthModal()" class="text-sm font-bold text-slate-600 hover:text-emerald-600 transition-colors focus:outline-none">Pengaduan</button>
                    @endauth
        ...
        <div id="mobile-menu" class="hidden md:hidden bg-white border-b border-slate-100 px-4 py-4 space-y-3 shadow-xl">
            ...
            <a href="#agenda" class="block text-base font-bold text-slate-700 hover:text-emerald-600">Agenda Kegiatan</a>
            @auth
                <a href="{{ route('warga.surat.index') }}" class="block text-base font-bold text-slate-700 hover:text-emerald-600">Surat Menyurat</a>
                <a href="{{ route('warga.pengaduan.index') }}" class="block text-base font-bold text-slate-700 hover:text-emerald-600">Pengaduan</a>
                <hr class="border-slate-100 my-2">
                <a href="{{ route('login') }}" class="block text-base font-bold text-slate-700 hover:text-emerald-600">Ke Dashboard</a>
            @else
                <button onclick="showAuthModal()" class="block w-full text-left text-base font-bold text-slate-700 hover:text-emerald-600 focus:outline-none">Surat Menyurat</button>
                <button onclick="showAuthModal()" class="block w-full text-left text-base font-bold text-slate-700 hover:text-emerald-600 focus:outline-none">Pengaduan</button>
                <hr class="border-slate-100 my-2">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                ...
                <div class="text-center lg:text-left">
                    <div class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs font-bold mb-6 backdrop-blur-sm shadow-sm">
                    ...
                    <h1 class="text-5xl lg:text-7xl font-extrabold text-slate-900 tracking-tight mb-6 leading-[1.1]">
                        Membangun <span class="text-transparent bg-clip-text bg-gradient-to-r from-emerald-500 to-teal-500">Rukun Tetangga</span> Yang Lebih Baik.
                    </h1>
                    ...
                    <div class="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4">
                        @auth
                            <a href="{{ route('login') }}" class="w-full sm:w-auto bg-emerald-500 hover:bg-emerald-600 text-white font-extrabold px-8 py-4 rounded-full transition-all duration-300 shadow-[0_0_20px_-5px_rgba(16,185,129,0.4)] hover:shadow-[0_0_25px_-5px_rgba(16,185,129,0.5)] hover:-translate-y-1 flex items-center justify-center gap-2">
                                Ke Dashboard Warga
                        ...
                        @else
                            ...
                            <a href="#mading" class="w-full sm:w-auto bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 font-bold px-8 py-4 rounded-full transition-all duration-300 shadow-sm hover:shadow-md flex items-center justify-center">
                                Lihat Pengumuman
                            </a>
                        @endauth
                    </div>
                ...
                <div class="hidden lg:block relative perspective-1000">
                    <div class="relative rounded-3xl overflow-hidden shadow-2xl border border-white/50 transform rotate-y-[-5deg] rotate-x-[5deg] hover:rotate-y-0 hover:rotate-x-0 transition-transform duration-700 ease-out bg-white p-4">
                        <img src="{{ asset('images/hero_illustration.jpg') }}" alt="Ilustrasi Komunitas RT" class="w-full h-auto object-cover rounded-2xl mix-blend-multiply opacity-95 hover:opacity-100 transition-opacity">
                    </div>
                        ...
                        <div class="bg-white rounded-3xl overflow-hidden border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(16,185,129,0.1)] transition-all duration-300 group hover:-translate-y-1 flex flex-col h-full">
                            @if($item->foto)
                                <div class="h-48 overflow-hidden relative">
                                    ...
                                    <img src="{{ Str::startsWith($item->foto, 'http') ? $item->foto : Storage::url($item->foto) }}" alt="{{ $item->judul }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                                </div>
                            @endif
    ...
    <section id="agenda" class="py-24 bg-white relative">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-16">
                ...
                <p class="mt-4 text-slate-500 max-w-2xl mx-auto font-medium">Jadwal kegiatan rutin dan khusus untuk warga RT 08 RW 02.</p>
            </div>
            ...
            @if(isset($kegiatan) && $kegiatan->isEmpty())
                <div class="bg-slate-50 rounded-3xl p-12 text-center border border-slate-100 shadow-sm">
                    ...
                    <p class="text-sm text-slate-500 mt-2">Belum ada agenda kegiatan yang dijadwalkan dalam waktu dekat.</p>
                </div>
            @elseif(isset($kegiatan))
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @foreach($kegiatan as $item)
                        ...
                        @endphp
                        <div class="bg-white rounded-3xl overflow-hidden border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(99,102,241,0.1)] transition-all duration-300 group hover:-translate-y-1 flex flex-col h-full relative cursor-pointer" onclick="openGallery({{ json_encode($fotos) }}, '{{ addslashes($item->nama_kegiatan) }}')">
                            @if($fotos && count($fotos) > 0)
                                <div class="h-48 overflow-hidden relative">
                                    <div class="absolute inset-0 bg-indigo-900/40 group-hover:bg-indigo-900/20 transition-colors z-10 flex items-center justify-center">
                                        <div class="opacity-0 group-hover:opacity-100 transition-all transform translate-y-4 group-hover:translate-y-0 bg-white/95 text-indigo-600 px-5 py-2.5 rounded-full font-bold text-sm shadow-xl flex items-center gap-2 backdrop-blur-sm">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                                            Lihat {{ count($fotos) }} Foto
                                        </div>
                                    </div>
                                    <img src="{{ Str::startsWith($fotos[0], 'http') ? $fotos[0] : Storage::url($fotos[0]) }}" alt="{{ $item->nama_kegiatan }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700">
                                </div>
                            @else
                                <div class="h-48 bg-gradient-to-br from-indigo-50 to-blue-50 flex items-center justify-center">
                            ...
                            <div class="p-8 flex-1 flex flex-col relative">
                                <div class="absolute -top-6 right-6 w-12 h-12 bg-white rounded-2xl shadow-lg border border-slate-100 flex flex-col items-center justify-center text-center">
                                    <span class="text-[10px] font-black uppercase text-slate-400 leading-none">{{ \Carbon\Carbon::parse($item->tanggal)->format('M') }}</span>
                                    <span class="text-lg font-extrabold text-indigo-600 leading-none mt-1">{{ \Carbon\Carbon::parse($item->tanggal)->format('d') }}</span>
                                </div>
                                <h4 class="text-xl font-bold text-slate-900 mb-3 group-hover:text-indigo-600 transition-colors line-clamp-2 pr-8 mt-2">
                                    {{ $item->nama_kegiatan }}
                                </h4>
                ...
                @if($kegiatan->count() >= 3)
                <div class="mt-12 text-center">
                    <a href="{{ route('login') }}" class="inline-flex items-center gap-2 bg-white border border-slate-200 text-slate-700 hover:text-indigo-600 hover:border-indigo-200 font-bold py-3 px-6 rounded-full transition-all text-sm shadow-sm hover:shadow-md">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                <div>
                    ...
                    <div class="space-y-6">
                        @auth
                        <a href="{{ route('warga.surat.index') }}" class="flex items-center gap-6 p-5 rounded-3xl hover:bg-white border border-transparent hover:border-slate-200 hover:shadow-xl transition-all cursor-pointer group">
                        @else
                        <div onclick="showAuthModal()" class="flex items-center gap-6 p-5 rounded-3xl hover:bg-white border border-transparent hover:border-slate-200 hover:shadow-xl transition-all cursor-pointer group">
                        @endauth
                            <div class="w-24 h-24 rounded-2xl bg-white shadow-sm group-hover:shadow-md group-hover:scale-110 transition-all flex items-center justify-center shrink-0 overflow-hidden border border-slate-100 relative">
                                <div class="absolute inset-0 bg-emerald-500/5 group-hover:bg-emerald-500/10 transition-colors z-10"></div>
                                <img src="{{ asset('images/surat_avatar.jpg') }}" class="w-full h-full object-cover mix-blend-multiply scale-110" alt="Surat Avatar">
                            </div>
                            <div>
                                <h4 class="text-xl font-bold text-slate-900 mb-1 group-hover:text-emerald-600 transition-colors">Pengajuan Surat Online</h4>
                                <p class="text-sm text-slate-500 leading-relaxed">Ajukan Surat Pengantar RT untuk berbagai keperluan administratif (KTP, Domisili, dll) langsung dari HP.</p>
                            </div>
                        @auth
                        </a>
                        @else
                        ...
                        @auth
                        <a href="{{ route('warga.pengaduan.index') }}" class="flex items-center gap-6 p-5 rounded-3xl hover:bg-white border border-transparent hover:border-slate-200 hover:shadow-xl transition-all cursor-pointer group">
                        @else
                        <div onclick="showAuthModal()" class="flex items-center gap-6 p-5 rounded-3xl hover:bg-white border border-transparent hover:border-slate-200 hover:shadow-xl transition-all cursor-pointer group">
                        @endauth
                            <div class="w-24 h-24 rounded-2xl bg-white shadow-sm group-hover:shadow-md group-hover:scale-110 transition-all flex items-center justify-center shrink-0 overflow-hidden border border-slate-100 relative">
                                <div class="absolute inset-0 bg-amber-500/5 group-hover:bg-amber-500/10 transition-colors z-10"></div>
                                <img src="{{ asset('images/pengaduan_avatar.jpg') }}" class="w-full h-full object-cover mix-blend-multiply scale-110" alt="Pengaduan Avatar">
                            </div>
                            <div>
                                <h4 class="text-xl font-bold text-slate-900 mb-1 group-hover:text-amber-500 transition-colors">Pengaduan Warga</h4>
                                <p class="text-sm text-slate-500 leading-relaxed">Sampaikan keluhan, aspirasi, atau masalah di lingkungan RT. Status penyelesaian dapat dipantau.</p>
                            </div>
                        @auth
                        </a>
                        @else
                        ...
                        <a href="#agenda" class="flex items-center gap-6 p-5 rounded-3xl hover:bg-white border border-transparent hover:border-slate-200 hover:shadow-xl transition-all cursor-pointer group">
                            <div class="w-24 h-24 rounded-2xl bg-white shadow-sm group-hover:shadow-md group-hover:scale-110 transition-all flex items-center justify-center shrink-0 overflow-hidden border border-slate-100 relative">
                                <div class="absolute inset-0 bg-indigo-500/5 group-hover:bg-indigo-500/10 transition-colors z-10"></div>
                                <img src="{{ asset('images/kegiatan_avatar.jpg') }}" class="w-full h-full object-cover mix-blend-multiply scale-110" alt="Kegiatan Avatar">
                            </div>
                            <div>
                                <h4 class="text-xl font-bold text-slate-900 mb-1 group-hover:text-indigo-500 transition-colors">Kegiatan Warga</h4>
                                <p class="text-sm text-slate-500 leading-relaxed">Dapatkan informasi jadwal kerja bakti, posyandu, atau pertemuan rutin RT agar tidak tertinggal.</p>
                            </div>
                        </a>
                    </div>
                </div>
                ...
                <div class="relative hidden lg:block">
                    ...
                    <div class="bg-white rounded-[3rem] shadow-xl border-8 border-white overflow-hidden h-auto w-full p-4 flex items-center justify-center">
                        <img src="{{ asset('images/hero_illustration.jpg') }}" alt="Features App" class="object-cover w-full h-full rounded-2xl mix-blend-multiply">
                    </div>
                </div>
            </div>
        </div>
    ...
    <div id="authModal" class="fixed inset-0 z-[100] hidden">
        <div class="absolute inset-0 bg-slate-900/60 backdrop-blur-sm transition-opacity" onclick="closeAuthModal()"></div>
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            <div class="relative bg-white rounded-3xl overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:max-w-md w-full border border-slate-100 opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95 duration-300" id="authModalContent">
                ...
                <div class="h-24 bg-gradient-to-r from-emerald-500 to-teal-500 relative">
                    ...
                    <button onclick="closeAuthModal()" class="absolute top-4 right-4 text-white/80 hover:text-white transition-colors focus:outline-none">
                ...
                <div class="px-8 pb-8 pt-0 relative">
                    <!-- Icon -->
                    <div class="w-20 h-20 rounded-2xl bg-white shadow-xl flex items-center justify-center mx-auto -mt-10 mb-6 border border-slate-50 relative z-10 transform rotate-3">
                        <div class="w-16 h-16 rounded-xl bg-amber-50 flex items-center justify-center border border-amber-100">
                    ...
                    <p class="text-slate-500 font-medium mb-8">Anda harus login terlebih dahulu untuk mengakses menu Surat Menyurat atau Pengaduan.</p>
                    
                    <div class="flex flex-col sm:flex-row gap-3">
                        <button onclick="closeAuthModal()" class="w-full sm:w-1/2 py-3 px-4 bg-slate-50 hover:bg-slate-100 text-slate-700 font-bold rounded-xl transition-colors border border-slate-200 focus:outline-none">
                            Tetap Disini
                        </button>
                        <a href="{{ route('login') }}" class="w-full sm:w-1/2 py-3 px-4 bg-emerald-500 hover:bg-emerald-400 text-slate-900 font-extrabold rounded-xl transition-all shadow-[0_0_20px_-5px_rgba(16,185,129,0.5)] hover:shadow-[0_0_25px_-5px_rgba(16,185,129,0.7)] hover:-translate-y-0.5 flex items-center justify-center gap-2">
                            Login Sekarang
                        </a>
                    </div>
                </div>
    ...
    <div id="galleryModal" class="fixed inset-0 z-[110] hidden">
        <div class="absolute inset-0 bg-slate-900/95 backdrop-blur-md transition-opacity" onclick="closeGallery()"></div>
        <button onclick="closeGallery()" class="absolute top-6 right-6 text-white/70 hover:text-white transition-colors focus:outline-none z-[120] bg-white/10 hover:bg-white/20 p-2 rounded-full">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
        </button>
        ...
        <div class="flex items-center justify-between h-full px-4 sm:px-12 pointer-events-none relative z-[115]">
            <button onclick="prevGalleryImage()" class="p-4 bg-white/10 hover:bg-white/20 rounded-full text-white pointer-events-auto transition-all backdrop-blur-md hover:scale-110 shadow-lg" id="btnPrev">
            ...
            <div class="w-full max-w-5xl h-[80vh] px-4 sm:px-12 flex items-center justify-center relative pointer-events-auto">
                <img id="galleryImage" src="" alt="Gallery" class="max-w-full max-h-full object-contain rounded-xl shadow-2xl transition-all duration-300">
            </div>

            <button onclick="nextGalleryImage()" class="p-4 bg-white/10 hover:bg-white/20 rounded-full text-white pointer-events-auto transition-all backdrop-blur-md hover:scale-110 shadow-lg" id="btnNext">
